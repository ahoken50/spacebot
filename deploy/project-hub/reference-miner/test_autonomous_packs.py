...
def main() -> None:
    temporary_root = Path(tempfile.mkdtemp(prefix="project-autonomous-packs-"))
    try:
        ...
        base_env = os.environ | {
            "PROJECT_HUB_WORKSPACE": str(workspace),
            "PROJECT_HUB_SKILLOPT_SKILL_CATALOG": str(SKILL_CATALOG),
        }
        dspy_env = base_env | {
            "PROJECT_HUB_OPTIMIZER_REFERENCE_PACK_PATH": str(dspy_pack),
            "PROJECT_HUB_OPTIMIZER_ALLOW_AUTONOMOUS_PACKS": "true",
        }
        skillopt_env = base_env | {
            "PROJECT_HUB_SKILLOPT_REFERENCE_PACK_PATH": str(skillopt_pack),
