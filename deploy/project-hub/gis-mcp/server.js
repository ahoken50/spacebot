...
const workspaceRoot = path.resolve(process.env.PROJECT_HUB_GIS_WORKSPACE ?? '/workspace');
...
function workspacePath(relativePath) {
  ...
  if (target !== workspaceRoot && !target.startsWith(`${workspaceRoot}${path.sep}`)) {
    throw new Error('Le chemin doit rester dans l’espace de travail Project Hub.');
  }
  return target;
}
...
function createServer() {
  const server = new McpServer({ name: 'project-gis-local', version: '0.1.0' });
  ...
  server.registerTool(
    ...
    async ({ kml_path }) => {
      ...
      return textResult({ kml_path, coordinate_reference_system: 'WGS84 (KML standard)', summary, named_features: named, note: 'Les totaux bruts incluent les hachures, symboles et géométries de conception. Utiliser project_surface_analysis avec des emprises projet validées pour les résultats programme de suivi.' });
    },
  );
  ...
  server.registerTool(
    'export_kml_geojson',
    {
      description: 'Convertir un KML local en GeoJSON avec les attributs de placemark et les métriques de superficie/longueur. Le fichier de sortie est créé dans le workspace Project Hub.',
      inputSchema: {
  ...
  server.registerTool(
    'project_surface_analysis',
    {
      ...
      inputSchema: {
        ...
        output_geojson_path: z.string().min(8).max(500).default('livrables/sig/analyse_emprises_project.geojson'),
      },
    },
    async ({ kml_path, project_boundaries_geojson_path, output_geojson_path }) => {
      ...
      return textResult({
        ...
        failures,
        important_limit: 'Cette somme est une superficie d’intersections brutes et peut compter des hachures, objets superposés ou objets de conception. L’agent doit classifier, dissoudre par type et faire une validation SIG humaine avant de déclarer une superficie programme de suivi finale.',
      });
    },
  );
...
app.listen(port, () => console.log(`Project Hub GIS MCP listening on port ${port}`));
