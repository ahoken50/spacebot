#!/usr/bin/env python3
"""Static integrity checks for the local Project Hub Spacebot deployment."""
...
assert mcp == {'project_shared_memory': 'http://project-shared-memory:3010/mcp'}, 'La mémoire commune doit être l’unique MCP partagé par défaut.'
expected_agent_mcp = {
    'project-coordination': {'project_document_studio', 'project_supervised_optimizer', 'project_skillopt', 'project_reference_miner'},
    'project-finance': {'project_document_studio'},
    'project-planning': {'project_document_studio'},
...
assert "process.env.PROJECT_HUB_OPTIMIZER_ENABLED ?? 'true'" in optimizer_server, 'Le serveur DSPy doit être actif par défaut.'
...
assert 'PROJECT_HUB_SKILLOPT_ENABLED: ${PROJECT_HUB_SKILLOPT_ENABLED:-true}' in compose, 'SkillOpt doit être actif par défaut.'
assert 'PROJECT_HUB_SKILLOPT_AUTONOMOUS_ENABLED: ${PROJECT_HUB_SKILLOPT_AUTONOMOUS_ENABLED:-true}' in compose, 'Le cycle SkillOpt autonome doit être configurable et actif par défaut.'
assert 'PROJECT_HUB_MEMORY_EXPORT_TOKEN: ${PROJECT_HUB_MEMORY_EXPORT_TOKEN:?Définir PROJECT_HUB_MEMORY_EXPORT_TOKEN dans .env}' in compose, 'L’export mémoire interne doit exiger un jeton local.'
assert 'PROJECT_HUB_REFERENCE_MINER_ENABLED: ${PROJECT_HUB_REFERENCE_MINER_ENABLED:-true}' in compose, 'Le mineur de références doit être actif par défaut.'
assert 'PROJECT_HUB_REFERENCE_MINER_AUTONOMOUS_ENABLED: ${PROJECT_HUB_REFERENCE_MINER_AUTONOMOUS_ENABLED:-true}' in compose, 'Le cycle autonome du mineur doit être configurable.'
assert 'PROJECT_HUB_AUTONOMOUS_PIPELINE_ENABLED: ${PROJECT_HUB_AUTONOMOUS_PIPELINE_ENABLED:-true}' in compose, 'Le pipeline autonome doit être activable dans Docker.'
assert 'PROJECT_HUB_AUTONOMOUS_PIPELINE_TOKEN: ${PROJECT_HUB_AUTONOMOUS_PIPELINE_TOKEN:?Définir PROJECT_HUB_AUTONOMOUS_PIPELINE_TOKEN dans .env}' in compose, 'Les déclenchements internes doivent être authentifiés.'
...
assert "PROJECT_HUB_SKILLOPT_ENABLED ?? 'true'" in skillopt_server, 'Le serveur SkillOpt doit être actif par défaut.'
...
assert 'PROJECT_HUB_SKILL_INSTALL_REQUIRE_APPROVAL' in install_skill_tool and 'approved_for_agent_install' in install_skill_tool, 'Le moteur doit bloquer install_skill sans autorisation Project Hub explicite.'
...
python_skill = ROOT / 'profile-skills' / 'project-python-workbench' / 'SKILL.md'
python_scaffold = ROOT / 'profile-skills' / 'project-python-workbench' / 'scripts' / 'scaffold_project_python_script.py'
...
for required in [
    ...
    ROOT / 'profile-skills' / 'project-coordination' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-finance-control' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-planning-governance' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-analysis' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-reporting' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-governance' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-document-studio' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-supervised-optimization' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-skillopt-learning' / 'SKILL.md',
    ROOT / 'profile-skills' / 'project-reference-case-mining' / 'SKILL.md',
