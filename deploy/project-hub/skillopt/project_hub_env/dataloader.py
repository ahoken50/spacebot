"""Data loader for Project Hub approved, redacted SkillOpt reference cases."""
...
class ProjectHubSkillDataLoader(SplitDataLoader):
    ...
    def load_split_items(self, split_path: str) -> list[dict[str, Any]]:
        ...
        if len(files) != 1:
            raise ValueError(f"Le répertoire {split_path} doit contenir exactement un fichier JSON de cas Project Hub.")
            ...
            items.append(
                {
                    ...
                    "task_type": str(raw.get("task_type") or "project_hub_skill"),
                }
            )
        return items
