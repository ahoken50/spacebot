...
description: Découvrir, réutiliser, installer ou créer une compétence ou un outil local de façon contrôlée. Utiliser lorsqu’une tâche Project Hub exige une capacité absente ou incertaine.
---
...
3. Si une capacité manque, définir le résultat attendu, les entrées/sorties, les contraintes de confidentialité, le coût estimé, la source autoritative et le niveau de risque. Chercher d’abord une compétence reconnue avec `skills_search(action="search")` ou une documentation officielle; ne pas exécuter de code récupéré sur le Web sans validation explicite.
4. Pour une compétence externe, vérifier sa description, ses dépendances, ses permissions et ses fichiers, puis partir de `templates/capability_skill_acquisition.template.json` pour écrire une proposition JSON `capability_skill_acquisition` dans `00_systeme/propositions_capacites/`. Elle doit limiter explicitement l’installation au workspace du profil et déclarer `false` pour les changements de MCP, modèle, Docker, permissions, secrets et dépendances système. Attendre la tâche `pending_approval` et **Approve** dans Spacebot avant `install_skill`. Les compétences externes restent installées et ne sont jamais modifiées automatiquement.
5. Si aucune compétence adaptée n’existe, créer une compétence **dans le workspace de l’agent** avec `skill_manage`, pas dans les compétences d’instance. Elle doit avoir un nom précis, une description de déclenchement, une procédure concise et seulement les ressources nécessaires. Pour un script associé, utiliser `project-python-workbench`, le classer dans `00_systeme/scripts/<agent-id>/`, le compiler et l’essayer sans modifier une source officielle.
