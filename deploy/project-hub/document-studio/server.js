...
const workspaceRoot = path.resolve(process.env.PROJECT_HUB_DOCUMENT_WORKSPACE ?? '/workspace');
const templatePath = '/app/templates/project-reference.docx';
...
function workspacePath(relativePath) {
  ...
  if (target !== workspaceRoot && !target.startsWith(`${workspaceRoot}${path.sep}`)) throw new Error('Le chemin doit rester dans l’espace de travail Project Hub.');
  return target;
}
...
async function renderMarkdown(sourceMarkdownPath, outputBasePath, outputFormats, title, author) {
  ...
  return { source_markdown_path: sourceMarkdownPath, outputs, template: 'project-reference.docx', note: 'Le contenu est resté local. Le modèle de langage ne rédige que le Markdown; Pandoc et LibreOffice réalisent le rendu local.' };
}
...
function createServer() {
  const server = new McpServer({ name: 'project-document-studio', version: '0.1.0' });

  server.registerTool(
    'get_document_taxonomy',
    {
      description: 'Afficher la taxonomie documentaire Project Hub et les dossiers autorisés avant de classer une source, une pièce de preuve ou un livrable.',
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    {
      description: 'Déplacer un fichier local vers un dossier explicitement autorisé par la taxonomie Project Hub. Refuse un écrasement et retourne le chemin final à enregistrer dans la mémoire commune.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    async ({ output_markdown_path, title, audience, objective, required_sections, source_references, output_formats }) => {
      ...
      const content = [
        ...
        '- Rendre le brouillon au format DOCX et/ou PDF avec la charte Project Hub.',
  ...
  server.registerTool(
    ...
    {
      description: 'Rendre un brouillon Markdown local en document DOCX et/ou PDF professionnel avec la charte Project Hub, table des matières et numérotation des sections.',
      inputSchema: {
        ...
        output_formats: z.array(z.enum(['docx', 'pdf'])).min(1).max(2).default(['docx', 'pdf']),
        title: z.string().min(3).max(250),
        author: z.string().min(2).max(200).default('organisation responsable'),
      },
    },
...
app.listen(port, () => console.log(`Project Hub document studio MCP listening on port ${port}`));
