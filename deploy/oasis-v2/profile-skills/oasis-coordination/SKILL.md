...
11. **Détection de lacune et proposition de nouvel agent** : Si une demande ou un nouveau type de tâche requiert une spécialité absente de l'équipe (ex: génie civil lourd, consultation citoyenne, subventions fédérales), ne pas improviser. Créer une proposition formelle avec l'outil de gestion des tâches (statut `pending_approval`, titre `Proposition de création d'agent : <Nom du profil>`) contenant l'analyse du besoin, l'invite système suggérée, les compétences et les accès nécessaires pour examen et approbation par l'administration municipale.
