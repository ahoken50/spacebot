...
Chaque profil utilise son **workspace privé** (`instance/agents/<agent-id>/workspace/`) pour ses compétences, essais, installations approuvées et fichiers de travail temporaires. Les six sandboxes reçoivent un unique chemin supplémentaire, `/data/shared-workspace`, pour lire et classer les sources, calculs et livrables communs. Cette séparation rend effectives les compétences spécialisées et empêche un agent de modifier directement le catalogue privé d’un autre profil.
...
L’image Spacebot OASIS contient **Python 3**, `venv` et `pip`; les six agents disposent du terminal sandboxé de Spacebot, borné à leur workspace privé avec le seul répertoire documentaire partagé explicitement autorisé. Ils peuvent donc créer et exécuter des scripts locaux à partir de l’interface Web, sans passer par un service externe. Chaque script doit être classé dans `00_systeme/scripts/<agent-id>/`, accompagné d’un README, compilé avec `python3 -m py_compile` et essayé sur une entrée non destructive avant de contribuer à un livrable. Le générateur `scaffold_oasis_python_script.py` fourni dans la compétence crée ce squelette avec des arguments explicites et des contrôles de chemin.

Les agents doivent d’abord inventorier les compétences, scripts, gabarits, binaires et MCP existants. Lorsqu’une compétence est réellement manquante, ils peuvent la rechercher dans le registre avec l’outil natif `skills_search`, mais ne doivent pas exécuter du code téléchargé. Ils préparent alors un fichier `capability_skill_acquisition` dans `00_systeme/propositions_capacites/`. Le pont transforme automatiquement ce fichier en une tâche `pending_approval` dans Spacebot. Après **Approve**, le pont écrit une autorisation dédiée sous `instance/skill-install-authorizations/`, hors des workspaces agents. Le moteur Spacebot autorise alors l’agent ciblé — et lui seul — à installer **exactement la source approuvée** dans son workspace avec `install_skill`; toute autre source est refusée lorsque l’instance OASIS est active. L’agent relit ensuite la compétence, l’essaie sur une tâche non sensible et consigne l’essai. Le pont n’installe aucun code et n’autorise jamais un MCP, un modèle, Docker, une permission, un secret ou une dépendance système.
...
> Une erreur de capacité ne permet pas d’installer ou de modifier automatiquement un outil, un MCP, Docker, un modèle, une permission ou la configuration. Le système produit une **leçon de diagnostic** à examiner; après son approbation, l’agent peut rechercher une compétence ou préparer une demande de capacité avec une source, des coûts et des tests vérifiables. Il ne fabrique jamais une installation ou un MCP sans ces éléments. Toute candidate, y compris une simple leçon d’instruction, reste `pending_approval` jusqu’à votre décision dans l’interface.
...
Les changements durables — ajout de conteneur, MCP, dépendance, modèle, secret, droit réseau ou permission — ne sont pas auto-activés. L’agent prépare alors une proposition classée dans `00_systeme/propositions_capacites/`, avec son origine, ses dépendances, ses coûts, ses tests et son plan de retrait. Le pont crée une tâche Spacebot `pending_approval`; seule une approbation explicite rend possible l’installation limitée de la source et du profil concernés. Une compétence installée, épinglée ou fournie par l’instance ne peut pas être modifiée automatiquement.
