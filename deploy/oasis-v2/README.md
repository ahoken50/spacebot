...
Pour l’activer, copier `00_systeme/optimisation/reference_cases.template.json` en `reference_cases.approved.json`, remplacer les exemples fictifs par un ou deux cas représentatifs puis obtenir l’approbation humaine en inscrivant le statut `approved`, le réviseur et la date. Le coordonnateur valide ensuite le jeu de référence et lance une proposition avec un seul candidat. Les métriques déterministes évaluent les termes essentiels, les assertions interdites, les marqueurs de source et la concision. Les propositions sont écrites sous `00_systeme/optimisation/propositions/` avec le statut `pending_approval`.
