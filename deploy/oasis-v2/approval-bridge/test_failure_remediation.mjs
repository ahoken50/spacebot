...
const instanceRoot = await mkdtemp(path.join(tmpdir(), 'oasis-approval-bridge-'));
...
await writeFile(path.join(proposalDir, 'proposal.json'), `${JSON.stringify({
  ...
  target_agent_id: 'oasis-finances',
...
const api = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/api/tasks?created_by=oasis-approval-bridge&limit=500') {
...
const child = spawn(process.execPath, ['server.js'], {
  ...
  env: {
    ...
    OASIS_WORKSPACE: workspace,
    OASIS_INSTANCE_ROOT: instanceRoot,
    OASIS_SPACEBOT_API_URL: `http://127.0.0.1:${apiPort}/api`,
    OASIS_APPROVAL_BRIDGE_TOKEN: token,
    OASIS_APPROVAL_BRIDGE_INTERVAL_SECONDS: '3600',
  },
...
try {
  ...
  const installed = await readFile(path.join(instanceRoot, 'agents', 'oasis-finances', 'workspace', 'skills', 'oasis-failure-lessons', 'SKILL.md'), 'utf8');
  const persisted = await readFile(path.join(instanceRoot, 'approved-skill-overlays', 'oasis-finances', 'oasis-failure-lessons', 'SKILL.md'), 'utf8');
