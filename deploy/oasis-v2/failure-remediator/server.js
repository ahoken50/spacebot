...
const enabled = !['0', 'false', 'no', 'off'].includes((process.env.OASIS_FAILURE_REMEDIATOR_ENABLED ?? 'true').trim().toLowerCase());
const intervalSeconds = Math.max(30, Number.parseInt(process.env.OASIS_FAILURE_REMEDIATOR_INTERVAL_SECONDS ?? '120', 10) || 120);
const maxProposalsPerDay = Math.max(1, Number.parseInt(process.env.OASIS_FAILURE_REMEDIATOR_MAX_PROPOSALS_PER_DAY ?? '3', 10) || 3);
const serviceToken = process.env.OASIS_FAILURE_REMEDIATOR_TOKEN ?? '';
const ownerAgentId = 'oasis-coordination';
const allowedAgents = new Set(['oasis-coordination', 'oasis-finances', 'oasis-calendrier', 'oasis-pse-sig', 'oasis-reddition', 'oasis-gouvernance']);
...
function proposalInstructions({ task, attempt, classification, signature }) {
  return [
    '---',
    `name: oasis-failure-lesson-${safeName(task.task_number)}-${signature.slice(0, 8)}`,
    'description: Prévenir la répétition d’un échec OASIS déjà observé. Utiliser lorsqu’une tâche ressemble au cas décrit dans cette leçon approuvée.',
    '---',
    '',
    '# Leçon de remédiation OASIS',
    '',
...
app.get('/healthz', (_req, res) => res.status(200).json({
  status: 'ok', service: 'oasis-failure-remediator', enabled, interval_seconds: intervalSeconds,
...
app.listen(port, '0.0.0.0', () => {
  console.log(`oasis-failure-remediator listening on ${port}`);
  if (enabled) {
