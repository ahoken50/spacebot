...
const memoryExportUrl = requiredEnvironment('OASIS_MEMORY_EXPORT_URL');
const memoryExportToken = requiredEnvironment('OASIS_MEMORY_EXPORT_TOKEN');
const enabled = isEnabled(process.env.OASIS_REFERENCE_MINER_ENABLED, true);
const autonomousEnabled = isEnabled(process.env.OASIS_REFERENCE_MINER_AUTONOMOUS_ENABLED, true);
...
function validatePolicy(policy) {
  ...
  if (policy.auto_promote === true) {
    throw new Error('La promotion automatique vers les packs actifs reste interdite dans OASIS.');
  }
  return policy;
}
...
function candidateFor(record, target) {
  ...
  if (target === 'dspy') {
    ...
    const agentId = redactText(payload.agent_id ?? 'oasis-coordination');
...
function createServer() {
  const server = new McpServer({ name: 'oasis-reference-miner', version: '0.1.0' });
...
const httpServer = app.listen(port, () => console.log(`OASIS reference-miner MCP server listening on port ${port}`));

async function shutdown(signal) {
  console.log(`Received ${signal}; shutting down OASIS reference-miner`);
  clearTimeout(timer);
