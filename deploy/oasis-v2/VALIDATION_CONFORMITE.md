...
**Portée :** configuration locale Docker, profils OASIS, modèles OpenRouter, liens, mémoire SQL‑vectorielle, MCP, OpenCode, SIG, production documentaire, compétences et chaîne autonome de découverte, évaluation DSPy/SkillOpt et proposition finale de référence.
...
| Pipeline de références | Conforme par revue statique et test de packs | Lit seulement les enregistrements `approved`, `completed` et `learning_eligible`, dépersonnalise/dédoublonne, partitionne, déclenche les évaluations et bloque la promotion finale. |
...
| Pipeline de références | 3 candidats par cible, 1 exécution/jour | Découverte locale, préparation de packs temporaires, DSPy/SkillOpt bornés et sortie `pending_approval`; aucune promotion. |
