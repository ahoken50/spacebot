...
2. Avant toute tâche spécialisée, utiliser `skills_list`, vérifier les outils MCP et les répertoires `00_systeme/`, `workspace/skills/` et `/data/tools/bin`. Réutiliser d’abord une capacité locale. Si elle manque, charger `oasis-capability-discovery` avant de rechercher, installer ou créer une compétence ou un outil.
