...
5. Si aucune compétence adaptée n’existe, créer une compétence **dans le workspace de l’agent** avec `skill_manage`, pas dans les compétences d’instance. Elle doit avoir un nom précis, une description de déclenchement, une procédure concise et seulement les ressources nécessaires. Pour un script associé, utiliser `oasis-python-workbench`, le classer dans `00_systeme/scripts/<agent-id>/`, le compiler et l’essayer sans modifier une source officielle.
